        ...
        <v-col cols="12" md="10" v-resize="onResize">
            ...
            >
                    <template v-slot:item="props">
                      <tr>
                        <td align="left">{{ props.item.acctNum }}</td>
                        <td align="left">{{ props.item.sub_Name }}</td>
                        <td nowrap="true" align="left">{{ props.item.sub_Description }}</td>
                        <td nowrap="true" align="left">{{ props.item.sub_Price }}</td>
                        <td nowrap="true" align="left">{{ props.item.acquired_date }}</td>
                        <td nowrap="true" align="left">{{ props.item.end_date }}</td>
                    ...
                    >
                      <v-card>
                        ...
                        <v-list v-show="isExpanded(item)" dense>
                          <v-list-item>
                            <v-list-item-content class="align-end">{{ item.sub_Description }}</v-list-item-content>
                          </v-list-item>
                          <v-list-item>
                            <v-list-item-content>Price:</v-list-item-content>
                            <v-list-item-content class="align-end">{{ item.sub_Price }}</v-list-item-content>
                          </v-list-item>
                          <v-list-item>
                            <v-list-item-content>Status:</v-list-item-content>
                            <v-list-item-content class="align-end">{{ item.acquired_date }} on {{ item.end_date }}</v-list-item-content>
                          </v-list-item>
                        </v-list>
                      </v-card>
                    </v-col>
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      isMobile: false,
      headers: [
        {text: 'Account Number', sortable: false, align: 'left',},
        {text: 'Subscription Name', sortable: false, align: 'left',},
